...
class Validation extends Fuel\Core\Validation
{
     ...
     * @author Bui Huu Phuc
     */
    public static function _validation_password1($val)
    {
        Validation::active()->set_message('password1', __('message.the_field_:label_is_invalid_password'));
        if (empty($val)) {
            return true;
        }
    ...
    public static function _validation_password($val)
    {
        ...
        return (boolean) preg_match('/^[\w]{4,}$/', $val);
    }
     ...
     * @author Dao Anh Minh
     */
    public static function _validation_auth_name($val)
    {
        Validation::active()->set_message('auth_name', 'Wrong format! Only characters, numbers and _ are accepted');
        return (boolean) preg_match('/^[a-z0-9]+[a-z0-9_]*[a-z0-9]$/', $val);
    }
     ...
     * @author Nguyen Van Hiep
     */
    public static function _validation_unique_field($val, $options, $obj)
    {
        ...
        Validation::active()->set_message('unique_field', __('message.this_:label_already_exists'));
     ...
     * @author Nguyen Van Hiep
     */
    public static function _validation_unique_area_perm($val, $obj)
    {
        $result = $obj->find('first', array(
            'where' => array(
                array('id', '!=', $obj['id']),
                array('area'       => Input::post('area')),
                array('permission' => Input::post('perm')),
            )
        ));
        Validation::active()->set_message('unique_area_perm', __('message.pair_exists'));
        return (boolean) (count($result) == 0);
    }
